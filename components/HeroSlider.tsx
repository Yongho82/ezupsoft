      ...
      <div className="relative w-full h-full z-10">
        <div className="max-w-[1440px] mx-auto h-full relative">
          ...
          {slides.map((slide, index) => (
            ...
            >
              <div className="w-full grid grid-cols-1 lg:grid-cols-[0.8fr_1.2fr] gap-8 lg:gap-16 items-center">

                {/* Text Content */}
                <div className="text-left pt-20 lg:pt-0 max-w-xl">
                  <h2 className="text-3xl md:text-5xl lg:text-5xl font-black text-white leading-[1.1] mb-4 md:mb-6 drop-shadow-2xl tracking-tight animate-fade-in-up">
                    {t(slide.titleKey)}
                  </h2>
                  <p className="text-base md:text-lg text-white/90 mb-6 md:mb-10 max-w-lg leading-relaxed font-medium drop-shadow-lg animate-fade-in-up">
                    {t(slide.descKey)}
                  </p>
                  <div className="animate-fade-in-up">
                    <NavLink to={slide.path}>
                      <Button size="lg" className={`w-full md:w-auto px-8 md:px-12 py-3 md:py-4 text-base md:text-lg font-bold shadow-2xl border-none hover:scale-105 transition-all rounded-2xl ${slide.accentColor}`}>
                        {t(slide.ctaKey)}
                      </Button>
                    </NavLink>
                  </div>
                ...
                <div className="hidden lg:block relative perspective-1000 flex items-center justify-end animate-fade-in-up" style={{ animationDelay: '0.3s' }}>
                  <div className="relative rounded-3xl overflow-hidden shadow-2xl border-[3px] border-white/10 transform rotate-y-[-8deg] rotate-x-[3deg] hover:rotate-0 transition-transform duration-700 ease-out aspect-[16/9] w-full max-w-[1000px] origin-right backdrop-blur-sm bg-white/5">
